...
Current Priority:
- MATCH-SKIP-TRACE-001 (COMPLETE 2026-08-23) — founder request: skip trace is
  wired into the Pairings UI, but strictly scoped to persisted MATCH-001
  pairing rows (never the raw property/buyer/owner universe), highest
  match_score first, bounded by an explicit user-supplied count, plus a
  manual per-pairing "Skip Trace" button. Every run through this path forces
  `DNC_PROVIDER=none` (DNC-MANUAL-001) regardless of the env default — numbers
...
|---|---|---|---|---|---|
| MATCH-SKIP-TRACE-001 | Codex/Tim | COMPLETE / IMPLEMENTED 2026-08-23 | Founder request: skip trace scoped strictly to persisted `MatchModel` ("Pairings") rows — never raw Assets/Owners/Buyers. `core/leadgen/match_skip_trace.py` resolves/creates a deterministic seller lead (keyed on `asset_id`, uuid5 `SELLER_NAMESPACE`) and buyer lead (keyed on `buyer_id`+representative-purchase-asset, same `BUYER_NAMESPACE`/formula as `scripts/run_skip_pilot.py`) per match, so the same property/buyer is never re-billed across repeat pairings. `run_top_pairings_skip_trace(limit)` traces the top N un-traced pairings by `match_score` descending; `run_match_skip_trace(match_id)` is the manual per-pairing trigger. Both always force `dnc_provider_name="none"` into `process_trace_job()` (DNC-MANUAL-001) regardless of the `DNC_PROVIDER` env default. New routes: `GET /api/matches/skip_trace/provider_status`, `GET /api/matches/{id}/contacts` (read-only), `POST /api/matches/{id}/skip_trace`, `POST /api/matches/skip_trace/bulk` (`server/routes_skip_trace.py`, capped at 100/request). UI (`ui/src/IntelView.jsx` `PairingsView`): a "# of pairings" input + "Skip trace top N" button above the table, a per-row "Skip Trace" button beside Details, and a provider-not-ready banner when `SKIPTRACE_API_KEY` isn't configured; phone numbers only render inside a pairing's expanded Details panel (never the main table row), labeled for manual DNC verification only. Fixed a real bug found while testing: `trace_jobs._ingest_result()` was not writing a `TraceResultModel` row for a provider "no match" response, so idempotency checks keyed on `TraceResultModel` existence (this module's `_already_traced()`) would resubmit — and re-bill — a lead forever if the vendor ever returned no match for it; now a `TraceResultModel` row with null match fields is always written. 2026-08-23 follow-up (Tim): most buyers on file are an LLC/trust name, not a person, and BatchData's skip trace is address-based by default (works without a name at all — see `BatchDataProvider` module docstring for how it resolves the individual behind an LLC-owned address on its own); but when the buyer participant's ingested `participant_type` (`adapters/real_estate/classification/entity_classifier.py`, set at ingestion, never re-derived here) is exactly `"individual"`, `match_skip_trace._buyer_confirm_name()` splits the name (`core.leadgen.normalization.split_person_name()`, returns `None` rather than guess for a single-token name) and stores it on the lead so `trace_jobs.py` passes it to BatchData's optional `name: {first, last}` request field as a match-confirmation hint (`TraceRequest.confirm_first_name`/`confirm_last_name`, `core/leadgen/providers/skiptrace/base.py`). Per BatchData's own documented behavior an unconfirmed name safely falls back to their default owner resolution rather than substituting a wrong person, which is what makes sending a best-effort split safe; an LLC/trust/entity buyer name is never split or sent. Seller side is untouched — BatchData's default address-only resolution already targets that exact property. Tests: `tests/test_leadgen_match_skip_trace.py` (including `BuyerConfirmNameTests`), updated `tests/test_leadgen_trace_jobs.py`, `tests/test_batchdata_provider.py` payload tests, `tests/test_leadgen_polish.py`::`test_split_person_name` | None — Railway currently has no production `SKIPTRACE_API_KEY` set, so live trace buttons will return `provider_not_ready` (409) until that's configured; the UI surfaces this via the provider-status banner | Skip trace can be run on-demand from the Pairings UI, bounded and dedup-safe, with manual DNC verification surfaced directly in the UI |
