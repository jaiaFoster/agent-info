...
|---|---|---|---|---|---|
| SPRINT-017 | Codex | CURRENT / WP1-WP3 IN PROGRESS | Hillsborough Evidence Completion: finish Hillsborough as the reference jurisdiction, classify every known live source, improve deterministic evidence/linking coverage, and keep source observability clear | SPRINT-016 complete; WP1 baseline run `30734899137` complete; WP2 assessment complete; DATA-020A/DATA-020B operationally verified; DATA-020C source verification complete; DATA-020D operationally verified; DATA-020E in progress to expose reusable financial-opportunity evidence for revenue review; FACT-LIVE-001 remains open and does not block evidence completion unless a live Sprint-017 consumer needs canonical facts | DATA-020E production validation; stronger seller evidence before SPRINT-018 revenue readiness |
| DATA-020E | Codex | CURRENT / IMPLEMENTATION PR | Financial Opportunity Evidence Surface: expose DATA-020D canonical `financial_opportunity.*` observations through read-only ops APIs with Asset attachment, amount/status/claim evidence, and provenance ids | DATA-020D operationally verified. No paid providers/outreach/recovery workflow; read-only API only. | Revenue-readiness review can inspect source-backed seller opportunity evidence instead of aggregate observation counts |
