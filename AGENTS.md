...
|---|---|---|---|---|---|
| SPRINT-017 | Codex | CURRENT / WP1-WP3 IN PROGRESS | Hillsborough Evidence Completion: finish Hillsborough as the reference jurisdiction, classify every known live source, improve deterministic evidence/linking coverage, and keep source observability clear | SPRINT-016 complete; WP1 baseline run `30734899137` complete; WP2 assessment complete; DATA-020A/DATA-020B operationally verified; DATA-020C source verification in progress; FACT-LIVE-001 remains open and does not block evidence completion unless a live Sprint-017 consumer needs canonical facts | DATA-020D candidate; stronger reference county before SPRINT-018 revenue readiness |
| DATA-020C | Codex | CURRENT / SOURCE VERIFIED IN PR | Tax Deed / Excess Proceeds Source Verification: verify exact public source, access contract, fields, legal/operational constraints, and ingestion feasibility before implementation | DATA-020A and DATA-020B complete; source assessment ranked tax-deed/excess-proceeds as high value but requiring source verification first. DATA-020C verified official Clerk page, direct Weekly Tax Deed Spreadsheet XLSX, direct claims/disbursement XLSX, PublicAccess JSON query 285, and reachable RealAuction sale calendar; no DB writes, no storage writes, no outreach. | Decide whether DATA-020D archive-first canonical ingestion is next highest-ROI WP3 source |
...
| DATA-020D | Codex | PLANNED / FOUNDER ROI DECISION NEEDED | Tax Deed Excess Proceeds Canonical Ingestion: archive and parse official Clerk weekly excess-proceeds XLSX, claims/disbursement XLSX, and bounded PublicAccess O&E/DR513 rows; emit SourceRecords/Observations/Events and link to Asset only by exact folio/HCPA evidence | DATA-020C source verification; founder confirms this seller-side source is next highest-ROI evidence target; no paid providers/outreach authorized | High-confidence surplus/excess-proceeds seller evidence and revenue-readiness review candidates |
